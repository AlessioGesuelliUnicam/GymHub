                                ...
                                @foreach($clients as $client)
                                    <tr class="bg-white border-b">
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4 w-1/12">
                                            @if (!is_null($client->med_cert_exp) && !is_null($client->med_cert))
                                                @php
                                                    $expirationDate = \Carbon\Carbon::parse($client->med_cert_exp);
                                                    $currentDate = \Carbon\Carbon::now();
                                                    $isExpired = $expirationDate->isPast();
                                                @endphp
                                                @if ($isExpired)
                                                    <span style="color: red;">
                                                         <a href="{{ asset('storage/'.$client->med_cert) }}"
                                                            target="_blank">{{ $expirationDate->format('d-m-Y') }}</a>
                                                     </span>
                                                @else
                                                    <a href="{{ asset('storage/'.$client->med_cert) }}"
                                                       target="_blank">{{ $expirationDate->format('d-m-Y') }}</a>
                                                @endif
                                            @elseif (!is_null($client->med_cert))
                                                <a href="{{ asset('storage/'.$client->med_cert) }}"
                                                   target="_blank">Si</a>
                                            @elseif (!is_null($client->med_cert_exp))
