                    ...
                    <div class="flex items-center justify-center h-24">
                        <button class="py-2 px-4 rounded-full hover:bg-gray-700">
                            <a href="{{ route('trainingSheets.create') }}">
                                <svg aria-hidden="true" class="w-6 h-6" fill="#f56565" viewBox="0 0 20 20"
            ...
            <div class="p-4 rounded-lg dark:border-gray-700 mt-16">
                <div class="grid grid-cols-2 mb-4 text-white bg-gray-800 rounded uppercase">
                    ...
                    <div class="flex items-center justify-center h-24">
                        <button class="py-2 px-4 rounded-full hover:bg-gray-700">
                            <a href="{{ route('trainingSheets.create') }}">
                                <svg aria-hidden="true" class="w-6 h-6 hover:fill-white" fill="#f56565"
                                ...
                                @foreach($data['clients'] as $key => $client)
                                    <tr class="bg-white border-b">
                                        ...
                                        <td class="whitespace-nowrap px-6 py-4 w-1/12">
                                            @if ($data['training_sheets'][$key]->training_sheet)
                                                <a href="{{ Storage::url($data['training_sheets'][$key]->training_sheet) }}" target="_blank"
                                        ...
                                        <td>
                                            <form method="POST" action="{{ route('trainingSheets.destroy', [$data['training_sheets'][$key]->id]) }}">
                                                ...
                                                <input class="bg-gray-800 text-white py-2 px-4 rounded-full right-0 hover:bg-red-500 cursor-pointer" type="submit" value="Elimina">
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
