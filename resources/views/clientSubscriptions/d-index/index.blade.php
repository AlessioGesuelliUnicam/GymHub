...
<div class="p-4 sm:ml-64">
    <div class="p-4 rounded-lg dark:border-gray-700">
        <div class="grid grid-cols-3 mb-4 text-white bg-gray-800 rounded uppercase">
            ...
            <div class="flex items-center justify-center h-24">
                <p class="text-2xl font-semibold text-xl p-6 ">Storico abbonamenti</p>
            </div>
            <div class="flex items-center justify-center h-24">
                <button class="py-2 px-4 rounded-full hover:bg-gray-700">
                    <a href="{{route('clientSubscriptions.create')}}">
                ...
                <div class="p-2 relative overflow-x-auto">
                    <table class="w-full text-center  text-sm font-light shadow-md table-auto">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-4 text-white w-1/12">Inizio Abbonamento</th>
                            <th scope="col" class="px-6 py-4 text-white w-1/12">Fine Abbonamento</th>
                            <th scope="col" class="px-6 py-4 text-white w-1/12">Elimina</th>
                        </thead>
                        <tbody>
                        @foreach($data['clients'] as $key => $client)
                        <tr class="bg-white border-b">
                            <td class="whitespace-nowrap   px-6 py-4 hidden w-1/12">{{$data['clientSubscriptions'][$key]->id}}</td>
                            <td class="whitespace-nowrap  px-6 py-4 w-1/12">{{$client['name']}}</td>
                            <td class="whitespace-nowrap  px-6 py-4 w-1/12">{{$client['surname']}}</td>
                            <td class="whitespace-nowrap  px-6 py-4 w-1/12">{{ $data['clientSubscriptions'][$key]->start_subscription }}</td>
                            <td class="whitespace-nowrap  px-6 py-4 w-1/12">{{ $data['clientSubscriptions'][$key]->end_subscription }}</td>
                            <td>
